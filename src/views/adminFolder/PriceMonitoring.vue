<template>
                  ...
                  <table>
                    <thead>
                      <tr>
                        ...
                        <th @click="sortTable('seller')">
                          Seller
                          <i class="fas fa-sort"></i>
                        </th>
                        <th @click="sortTable('currentPrice')">
                          Current Price
                          <i class="fas fa-sort"></i>
                        </th>
                    ...
                    <tbody>
                      <tr v-for="(product, index) in filteredProducts" :key="index">
                        ...
                        <td>
                          <div class="volatility-indicator" :class="getVolatilityClass(product.volatility)">
                            {{ getVolatilityLabel(product.volatility) }}
                          </div>
                        </td>
                        <td>
        ...
        <div class="modal-content" @click.stop>
          ...
          <div class="modal-body" v-if="selectedProduct">
            <div class="product-details">
              <div class="product-image-large" :style="{ backgroundImage: `url(${selectedProduct.image})` }"></div>
              <div class="product-info">
                ...
                <div class="product-meta">
                  ...
                  <div class="meta-item">
                    ...
                    <span class="meta-value">{{ selectedProduct.seller }}</span>
                  </div>
                  <div class="meta-item">
                    <span class="meta-label">Current Price:</span>
                    <span class="meta-value">${{ selectedProduct.currentPrice.toFixed(2) }}</span>
                  </div>
                  <div class="meta-item">
            ...
            <div class="price-statistics">
              <div class="stat-card">
                <h4>Highest Price</h4>
                <div class="stat-value">${{ selectedProduct.highestPrice.toFixed(2) }}</div>
                <div class="stat-date">{{ formatDate(selectedProduct.highestPriceDate) }}</div>
              </div>
              <div class="stat-card">
                <h4>Lowest Price</h4>
                <div class="stat-value">${{ selectedProduct.lowestPrice.toFixed(2) }}</div>
                <div class="stat-date">{{ formatDate(selectedProduct.lowestPriceDate) }}</div>
              </div>
              <div class="stat-card">
                <h4>Average Price</h4>
                <div class="stat-value">${{ selectedProduct.averagePrice.toFixed(2) }}</div>
                <div class="stat-date">Last 30 days</div>
              </div>
              <div class="stat-card">
                <h4>Price Volatility</h4>
                <div class="stat-value">{{ getVolatilityLabel(selectedProduct.volatility) }}</div>
                <div class="volatility-indicator" :class="getVolatilityClass(selectedProduct.volatility)"></div>
              </div>
            </div>
            ...
            <div class="price-history-table">
              ...
              <table>
                ...
                <tbody>
                  <tr v-for="(entry, index) in selectedProduct.priceHistory" :key="index">
                    <td>{{ formatDate(entry.date) }}</td>
                    <td>${{ entry.price.toFixed(2) }}</td>
                    <td :class="getPriceChangeClass(entry.change)">
                      {{ formatPriceChange(entry.change) }}
                    </td>
                    <td>{{ entry.notes || '-' }}</td>
                  </tr>
                </tbody>
  ...
  export default {
    ...
    setup() {
      ...
      // Summary data
      const avgPriceChange = ref('+2.8%');
      const productsMonitored = ref(248);
      const categoriesCount = ref(12);
      const priceAlerts = ref(18);
      const newAlertsToday = ref(3);
      const sellersCount = ref(42);
      const activeSellersCount = ref(38);
      ...
      const generateSampleData = () => {
        ...
        const productNames = {
          vegetables: ['Organic Tomatoes', 'Sweet Corn', 'Bell Peppers', 'Carrots', 'Spinach', 'Potatoes', 'Onions', 'Broccoli'],
          fruits: ['Strawberries', 'Apples', 'Bananas', 'Oranges', 'Grapes', 'Watermelon', 'Blueberries', 'Mangoes'],
          dairy: ['Milk', 'Cheese', 'Yogurt', 'Butter', 'Cream', 'Cottage Cheese'],
          grains: ['Rice', 'Wheat Flour', 'Oats', 'Barley', 'Quinoa', 'Corn Flour'],
          meat: ['Chicken', 'Beef', 'Pork', 'Lamb', 'Turkey', 'Fish']
        };
        
        const sellers = ['Farm Fresh Co.', 'Green Valley Farms', 'Sunrise Organics', 'Nature\'s Best', 'Harvest Fields', 'Country Goods'];
        
        // Generate products for each category
        Object.keys(productNames).forEach(category => {
          productNames[category].forEach(name => {
            const basePrice = Math.random() * 15 + 2; // Random price between $2 and $17
            const priceChange = (Math.random() * 10) - 5; // Random change between -5% and +5%
            const previousPrice = basePrice * (1 - (priceChange / 100));
            const volatility = Math.random(); // Random volatility between 0 and 1
            ...
            for (let i = 30; i >= 0; i--) {
              ...
              const dailyChange = (Math.random() * 4) - 2; // Random daily change between -2% and +2%
              const price = i === 0 ? basePrice : lastPrice * (1 + (dailyChange / 100));
              
              priceHistory.push({
                date: date.toISOString(),
                price: price,
                change: i === 30 ? 0 : ((price - priceHistory[priceHistory.length - 1].price) / priceHistory[priceHistory.length - 1].price) * 100,
                notes: i % 10 === 0 ? 'Seasonal adjustment' : ''
              });
              
              lastPrice = price;
            }
            
            // Calculate highest, lowest, and average prices
            const prices = priceHistory.map(entry => entry.price);
            const highestPrice = Math.max(...prices);
            const lowestPrice = Math.min(...prices);
            const averagePrice = prices.reduce((sum, price) => sum + price, 0) / prices.length;
            
            const highestPriceEntry = priceHistory.find(entry => entry.price === highestPrice);
            const lowestPriceEntry = priceHistory.find(entry => entry.price === lowestPrice);
            
            sampleProducts.push({
              id: `${category}-${name.toLowerCase().replace(/\s+/g, '-')}`,
              name: name,
              category: categories.value.find(c => c.id === category).name,
              ...
              hasAlert: Math.random() > 0.8, // 20% chance of having an alert
              priceHistory: priceHistory,
              highestPrice: highestPrice,
              highestPriceDate: highestPriceEntry.date,
        ...
        const sampleAlerts = [];
        const alertTypes = ['Price Increase', 'Price Decrease', 'High Volatility', 'Threshold Exceeded'];
        ...
        for (let i = 0; i < 5; i++) {
          ...
          switch (alertType) {
            ...
            case 'High Volatility':
              message = `Unusual price volatility detected. Multiple price changes in 24 hours.`;
              break;
            case 'Threshold Exceeded':
              message = `Price exceeded the set threshold of $${(randomProduct.currentPrice * 1.2).toFixed(2)}.`;
              break;
          }
      ...
      const filteredProducts = computed(() => {
        ...
        if (selectedCategory.value) {
          result = result.filter(product => 
            product.category === categories.value.find(c => c.id === selectedCategory.value).name
          );
        }
      ...
      const renderChart = () => {
        ...
        if (currentChartView.value === 'category') {
          ...
          const days = parseInt(selectedTimePeriod.value);
          const today = new Date();
          ...
          for (let i = days; i >= 0; i--) {
            ...
            categories.value.forEach(category => {
              const categoryProducts = products.value.filter(p => p.category === category.name);
              if (categoryProducts.length > 0) {
                // Find average price for this category on this day
                let sum = 0;
                categoryProducts.forEach(product => {
                  ...
                  if (historyEntry) {
                    sum += historyEntry.price;
                  }
                });
                const avgPrice = sum / categoryProducts.length;
                categoryData[category.name].push(avgPrice);
              } else {
                categoryData[category.name].push(0);
              }
            });
          }
          ...
          // Group by seller
          const sellerData = {};
          const uniqueSellers = [...new Set(products.value.map(p => p.seller))];
          
          uniqueSellers.forEach(seller => {
            sellerData[seller] = [];
          });
          
          // Generate data points for each seller
          const days = parseInt(selectedTimePeriod.value);
          const today = new Date();
          ...
          for (let i = days; i >= 0; i--) {
            ...
            uniqueSellers.forEach(seller => {
              const sellerProducts = products.value.filter(p => p.seller === seller);
              if (sellerProducts.length > 0) {
                // Find average price for this seller on this day
                let sum = 0;
                ...
                const avgPrice = sum / sellerProducts.length;
                sellerData[seller].push(avgPrice);
              } else {
                sellerData[seller].push(0);
              }
            });
          }
          
          // Create datasets for each seller
          ...
          // Show top 5 products by price change
          const topProducts = [...products.value]
            .sort((a, b) => Math.abs(b.change) - Math.abs(a.change))
            .slice(0, 5);
          
          // Generate data points for each product
          const days = parseInt(selectedTimePeriod.value);
          const today = new Date();
          ...
          topProducts.forEach((product, index) => {
            ...
            for (let i = days; i >= 0; i--) {
              ...
              if (historyEntry) {
                productData.push(historyEntry.price);
              } else {
      ...
      const renderProductChart = () => {
        ...
        // Prepare data
        const labels = selectedProduct.value.priceHistory.map(entry => formatDate(entry.date));
        const prices = selectedProduct.value.priceHistory.map(entry => entry.price);
      ...
      const showMoreOptions = (product) => {
        ...
        alert(`More options for ${product.name}`);
      };
      ...
      const exportPriceHistory = (product) => {
        ...
        alert(`Exporting price history for ${product.name}`);
      };
      
      const setupPriceAlert = (product) => {
        // This would show a modal to set up a price alert
        alert(`Setting up price alert for ${product.name}`);
      };
